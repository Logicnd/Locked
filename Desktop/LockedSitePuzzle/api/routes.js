...
// Admin: Reset leaderboard (requires special key)
router.post('/leaderboard/reset', requireAuth, async (req, res) => {
  try {
    const { resetKey, scope = 'all' } = req.body;
    const { getAllUsers, saveUser } = require('../server');
    
    // Verify reset key (should be in env or admin config)
    const validResetKey = process.env.LEADERBOARD_RESET_KEY || 'admin-reset-key';
    ...
    const resetCount = { total: 0, phase2: 0, phase3: 0 };
    
    for (const user of users) {
      let modified = false;
      
      if (scope === 'all' || scope === 'phase2') {
        if (user.puzzle2_solved) {
      ...
      if (scope === 'all' || scope === 'phase3') {
        if (user.puzzle3_solved) {
      ...
      if (scope === 'all') {
        user.xp = Math.min(user.xp || 0, 100); // Keep minimal XP
        user.badges = [];
        user.level = 1;
        resetCount.total++;
        modified = true;
